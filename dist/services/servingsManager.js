...
export class ServingsManager {
    constructor(recipe, servingsInput, previewElem) {
        this._recipe = recipe;
        ...
        if (!this._servingsInput.leadBtn)
            throw new Error("Missing lead button");
        this._servingsInput.leadBtn.addEventListener("click", () => {
            this.scheduleRecalc();
        });
        if (!this._servingsInput.trailBtn)
            throw new Error("Missing trail button");

...
export class RecipeStorage {
  ...
  async getByKeyword(keyword: string): Promise<Recipe[]> {
    await this.getRecipes();
  ...
  async search(filters: {
    ...
    if (filters.dietTags && filters.dietTags.length > 0) {
      ...
      results = results.filter((r) =>
        r.dietTags.some((t) => lowerCategories.includes(t.toLowerCase())),
      );
    }
    ...
    if (filters.mealTypeTags && filters.mealTypeTags.length > 0) {
      ...
      results = results.filter((r) =>
        r.mealTypeTags.some((t) => lowerCategories.includes(t.toLowerCase())),
      );
    }
    return results;
  }
